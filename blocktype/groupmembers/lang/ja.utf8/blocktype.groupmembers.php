<?php
/**
 *
 * @package    mahara
 * @subpackage lang (Japanese)
 * @translator Mitsuhiro Yoshida (https://mitstek.com/)
 * @started    2008-01-19 11:25:00 UTC
 * @updated    2026-06-04 07:08:32 UTC
 * @license    https://www.gnu.org/licenses/gpl-3.0.html GNU GPL version 3 or later
 * @copyright  For copyright information on Mahara, please see the README file distributed with this software.
 *
 */

defined('INTERNAL') || die();

$string['title'] = 'グループメンバ';
$string['title_help'] = '<h1>セクションタイトル</h1><p>このフィールドを空白のままにした場合、タイトルが生成されます。</p>';
$string['description'] = 'このグループのメンバ一覧を表示します。';
$string['show_all'] = 'このグループのメンバすべてを表示する';
$string['options_numtoshow_title'] = '表示メンバ';
$string['numtoshow_help'] = '<h1>表示メンバ</h1><p>あなたが表示したいメンバ数を選択してください。</p>';
$string['options_order_title'] = '表示順';
$string['order_help'] = '<h1>表示順</h1><p>あなたはメンバを登録日順 (新しい順) またはランダムな順序で表示できます。';
$string['Latest'] = '最新';
$string['Random'] = 'ランダム';
