<?php
/**
 *
 * @package    mahara
 * @subpackage lang (Japanese)
 * @translator Mitsuhiro Yoshida (https://mitstek.com/)
 * @started    2008-01-19 11:25:00 UTC
 * @updated    2026-05-22 13:30:06 UTC
 * @license    https://www.gnu.org/licenses/gpl-3.0.html GNU GPL version 3 or later
 * @copyright  For copyright information on Mahara, please see the README file distributed with this software.
 *
 */

defined('INTERNAL') || die();

$string['title1'] = '個人情報';
$string['description1'] = '表示する個人情報を選択してください。';
$string['title_help'] = '<h1>セクションタイトル</h1><p>ヘッディングを表示したくない場合、あなたはタイトルを空白のままにできます。</p>';
$string['aboutme'] = '私について';
$string['fieldstoshow'] = '表示するフィールド';
$string['introtext'] = '自己紹介テキスト';
$string['introtext_help'] = '<h1>自己紹介テキスト</h1><p>上の「自己紹介」プロファイルフィールドの文章を使用したくない場合、ここにあなた独自の自己紹介文を入力してください。</p>';
$string['dontshowprofileicon'] = 'プロファイル画像を表示しません。';
$string['showdefaultprofileicon'] = 'プロファイル画像を表示します。';
$string['dontshowemail'] = 'メールアドレスを表示しません。';
$string['uploadaprofileicon'] = 'あなたにはプロファイル画像がありません。<a href="%sartefact/file/profileicons.php">アップロードしてください</a>。';
$string['noprofilesselectone'] = 'プロファイルアイテムが選択されていません。';
$string['dontshowsocialprofiles'] = 'ソーシャルメディアアカウントを表示しない';
$string['showsocialprofiles'] = '選択したソーシャルメディアアカウントを表示する';
