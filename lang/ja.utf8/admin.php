 ...
 * @updated    2020-03-09 12:43:38 UTC
...
$string['allowediframesitesdescriptiondetail'] = '外部メディアブロック設定時、作成者にアイコンおよび表示名が表示されます。同一表示名のサイトすべてが設定フォーム内でグルーピングされますが同一サイト内におけるiframeソーステキストの一部合致は許可されます。';
...
<p>あなたが週ごとの更新状況の送信を決定した場合、Maharaプロジェクトに送信される予定の情報を確認できます。あなたのサイトのすべての人に関して個人を特定されるものは一切送信されません。</p>';
...
$string['tagssideblockmaxtagsdescription'] = 'タグクラウド内に表示されるタグのデフォルト数です。';
...
$string['linksandresourcesmenupagedescription'] = '「リンクおよびリソース」メニューはほとんどのページですべての人に表示されます。あなたは他のウェブサイトのリンクおよび%s管理ファイル%sセクションでアップロードしたファイルへのリンクを追加できます。';
...
$string['networkingpagedescription'] = 'Maharaのネットワーキング機能では同じサーバまたは別のサーバで動作しているMaharaまたはMoodleと連携できます。ネットワーキングが有効にされた場合、MoodleまたはMaharaにログインした人のシングルサインオンを設定できます。';
...
$string['uploadcsvinstitution'] = '新しいアカウントのインスティテューションおよび認証方法';
$string['configureauthplugin'] = 'あなたが人を追加するには最初に認証プラグインを設定する必要があります。';
$string['csvfiledescription'] = '追加する人を含むファイル';
