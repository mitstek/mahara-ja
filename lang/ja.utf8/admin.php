...
$string['missingplugindisabled'] = 'インストール済みプラグイン (%s) が見つからなかったため、無効にされました。';
...
$string['allowgroupcategoriesdescription'] = 'この設定を有効にした場合、サイト管理者はユーザがグループに割り当てることのできるカテゴリを作成できます。';
...
$string['cronnotrunning1'] = '<span class="error">cronが動作していません。</span><br>cronの設定に関するインストラクションは<a href="https://wiki.mahara.org/wiki/System_Administrator%27s_Guide/Installing_Mahara">installation guide</a>をご覧ください。あなたがすでにcronを設定している場合、直近の1つまたはそれ以前の処理が正しく実行されませんでした。';
...
$string['allowpublicviewsdescription'] = 'この設定を有効にした場合、ログインユーザだけではなく、その他のユーザからもアクセス可能なポートフォリオページを作成できます。';
$string['allowinstitutionpublicviews'] = 'インスティテューションパブリックページを許可する';
$string['allowinstitutionpublicviewsdescription'] = 'この設定を有効にした場合、このインスティテューションに属しているユーザはログインユーザだけではなく、その他のユーザからもアクセス可能なポートフォリオページを作成できます。';
$string['allowpublicprofiles'] = 'パブリックプロファイルを許可する';
$string['allowpublicprofilesdescription'] = 'この設定を有効にした場合、ログインユーザだけではなく、その他のユーザからもアクセスできるようプロファイルページを設定できます。';
$string['anonymouscomments'] = '匿名コメント';
$string['anonymouscommentsdescription'] = 'この設定を有効にした場合、パブリックページまたはシークレットURLアクセス可能なページに対して誰でもコメントを投稿できます。';
...
$string['dropdownmenudescription'] = 'この設定を有効にした場合、メインMaharaナビゲーションのサブナビゲーションにドロップダウンフォーマットを使用します。';
...
$string['showonlineuserssideblockdescriptionmessage'] = 'この設定を有効にした場合、ユーザに対してユーザにオンラインユーザ一覧のサイドバーが表示されます。';
...
$string['showtagssideblockdescription1'] = 'この設定を有効にした場合、ユーザに対して人は使用頻度の高いタグ一覧のサイドバーをサイト内の一部の場所で確認できます。';
$string['simple'] = 'シンプル';
...
$string['staffuserreportsdescription'] = 'この設定を有効にした場合、インスティテューション内ユーザのレポートページにサイトおよびインスティテューションスタッフがアクセスできるようになります。通常、このページは管理者に制限されページアクセスリストを含むユーザ詳細情報が一覧表示されます。';
$string['staffuserstats'] = 'スタッフ統計アクセス';
$string['staffuserstatsdescription'] = 'この設定を有効にした場合、インスティテューション内のユーザ統計ページにインスティテューションスタッフがアクセスできるようになります。通常、このページは管理者およびサイトスタッフにのみ制限されています。';
...
$string['usersallowedmultipleinstitutionsdescription'] = 'この設定を有効にした場合、ユーザは同時に複数のインスティテューションのメンバになることができます。';
$string['userscanchooseviewthemes'] = 'ユーザがページテーマを選択できる';
$string['userscanchooseviewthemesdescription'] = 'この設定を有効にした場合、ユーザはページの編集時、テーマを選択できます。選択したテーマを使用して他のユーザにページが表示されます。';
$string['userscanhiderealnames'] = 'ユーザは実名を秘匿できる';
$string['userscanhiderealnamesdescription'] = 'この設定を有効にした場合、表示名を設定したユーザは表示名のみ検索対象として実名を検索されないようにできます (サイト管理セクションでは常にユーザを実名で検索できます)。';
$string['usersseenewthemeonlogin'] = '次回ログイン時、他のユーザに新しいテーマが表示されます。';
$string['viruschecking'] = 'ウイルスチェック';
$string['viruscheckingdescription'] = 'この設定を有効にした場合、すべてのアップロードファイルへのClamAVを使用したウイルスチェックが有効にされます。';
...
$string['adminfilespagedescription'] = 'あなたはここで%s「リンクおよびリソース」メニュー%sに含めれるファイルをアップロードできます。パブリックディレクトリのファイルはパブリックの「リンクおよびリソース」メニューに追加できるのに対してホームディレクトリのファイルはログイン後の「リンクおよびリソース」メニューに追加できます。';
...
$string['uploadgroupmemberscsvpagedescription2'] = '<p>あなたはこの機能を使用してこのインスティテューションで管理されるグループ内のグループメンバを更新できます。あなたは<acronym title="Comma Separated Values">CSV</acronym>ファイルで新しいメンバをアップロードできます。</p>
...
$string['noleap2axmlfiledetected'] = 'leap2a.xmlファイルは検出されませんでした - 再度あなたのエクスポートファイルを確認してください。';
$string['Or...'] = 'または ...';
...
$string['licensemandatorydescription'] = 'この設定を有効にした場合、ユーザはアーティファクト作成時にライセンスを選択する必要があります。そうではない場合、ライセンスフィールドを空白のままにできます。';
...
$string['institutionsettingsdescription'] = 'ここであなたはシステム内にあるこのユーザのインスティテューションメンバシップ設定を変更できます。';
...
$string['institutionusersinstructionsrequesters'] = '左側のユーザ一覧にはあなたのインスティテューションに参加をリクエストした、すべてのユーザが表示されます。あなたは表示されるユーザ数を減らすため検索ボックスを使用できます。あなたがユーザをインスティテューションに追加したい場合、またはユーザのメンバシップリクエストを拒否したい場合、1名またはそれ以上のユーザを選択して右矢印をクリックすることで最初にユーザを右側ボックスに移動してください。「メンバを追加する」ボタンをクリックして右側ボックスのすべてのユーザをインスティテューションに追加できます。「リクエストを拒否する」ボタンをクリックして右側ボックスのユーザのメンバシップリクエストを拒否できます。';
$string['institutionusersinstructionsnonmembers'] = '左側のユーザ一覧にはあなたのインスティテューションのメンバではない、すべてのユーザが表示されます。あなたは表示されるユーザ数を減らすため検索ボックスを使用できます。あなたがユーザに対してインスティテューションへの参加を招待したい場合、1名またはそれ以上のユーザを選択して右矢印をクリックすることで最初にユーザを右側ボックスに移動してください。「ユーザを招待する」ボタンをクリックして右側ボックスのすべてのユーザに招待を送信できます。招待を受け入れるまでこれらのユーザはインスティテューションと関連付けされません。';
$string['institutionusersinstructionslastinstitution'] = '左側のユーザ一覧にはあなたのインスティテューションのメンバではなく、選択されたインスティテューションを離れたすべてのユーザが表示されます。あなたは表示されるユーザ数を減らすため検索ボックスを使用できます。あなたがユーザに対してインスティテューションへの参加を招待したい場合、1名またはそれ以上のユーザを選択して右矢印をクリックすることで最初にユーザを右側ボックスに移動してください。「ユーザを招待する」ボタンをクリックして右側ボックスのすべてのユーザに招待を送信できます。招待を受け入れるまでこれらのユーザはインスティテューションと関連付けされません。';
$string['institutionusersinstructionsmembers'] = '左側のユーザ一覧にはあなたのインスティテューションのすべてのユーザが表示されます。あなたは表示されるユーザ数を減らすため検索ボックスを使用できます。ユーザをインスティテューションから削除するには最初に1名またはそれ以上のユーザを左側のボックスで選択した後、右矢印をクリックしてください。あなたが選択したユーザが右側のボックスに移動されます。「ユーザを削除する」ボタンをクリックして右側ボックスのすべてのユーザが削除されます。左側ボックスのユーザはインスティテューションに参加したままの状態です。';
$string['institutionusersinstructionsinvited'] = '左側のユーザ一覧にはインスティテューションへの参加の招待が送信されたすべてのユーザおよび誰が承認または拒否したか表示されます。あなたは表示されるユーザ数を減らすため検索ボックスを使用できます。インスティテューションへの招待を取り消すには最初に1名またはそれ以上のユーザを左側のボックスで選択した後、右矢印をクリックしてください。あなたが選択したユーザが右側のボックスに移動されます。「招待を取り消す」ボタンをクリックして右側ボックスのすべてのユーザへの招待が取り消されます。左側ボックスのユーザはインスティテューションに招待されたままの状態でいつでもインスティテューションに参加できます。';
...
$string['noinstitutionsstatsdescription'] = 'あなたのユーザに利用できるインスティテューションがないためインスティテューション統計を利用できません。';
...
$string['updateinstitutionuserquotasdesc'] = 'この設定を有効にした場合、あなたが上で選択したデフォルトクオータを既存のメンバすべてに適用します。';
$string['approve'] = '承認';
$string['deny'] = '拒否';
...
$string['managegroupquotadescription'] = 'このグループのグループファイルクオータを変更するためこのフォームを使用してください。';
...
$string['userscandisabledevicedetectiondescription'] = 'この設定を有効にした場合、ユーザはこのサイト閲覧時のモバイルデバイス検出を無効にできます。';
...
$string['masqueradingreasonrequireddescription'] = 'この設定を有効にした場合、管理者は別のユーザとしてマスカレーディングする時に理由を入力する必要があります。この理由は記録されます。また、「ユーザにマスカレーディングを通知する」設定が有効にされた場合、マスカレーディングに関する通知がユーザにも送信されます。';
$string['masqueradingnotified'] = 'ユーザにマスカレーディングを通知する';
$string['masqueradingnotifieddescription'] = 'この設定を有効にした場合、管理者がユーザをマスカレーディングした時に通知されます。通知には誰が、いつ、なぜ (「マスカレーディングの理由を要求する」が有効の場合) マスカレーディングしたのか記載されます。';
