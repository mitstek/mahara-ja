...
$string['staffaccessreportsdescription1'] = 'サイトおよびインスティテューションスタッフがインスティテューションの「アカウント詳細」「ポートフォリオアクセス」および「マスカレーディングセッション」レポートにアクセスできるようにします。通常、これらのレポートは管理者に制限されています。';
...
$string['uploadcsvsomeuserscouldnotbeemailed'] = '一部の人にメール送信できませんでした。メールアドレスが有効ではないかMaharaが動作しているサーバがメールを適切に送信できるように設定されていません。サーバのエラーログにはさらに詳細が記録されています。現在のところ、これらの人には手動で連絡してください:';
...
$string['loginasoverridepasswordchange'] = 'あなたは別の人としてログインしているためパスワード変更画面を無視して%sとにかくログインすること%sを選択できます。';
...
$string['registrationalloweddescription5'] = '人は登録フォームを使用してこのインスティテューションに登録できます。登録が無効にされた場合、非メンバはインスティテューションのメンバシップをリクエストできません。また、メンバは自分でインスティテューションを離れたりアカウントを削除できません。';
...
$string['showonlineusersdescription'] = 'このインスティテューションのメンバに表示されるオンラインの人です。人に複数インスティテューションが許可されている場合、そしてインスティテューションの設定が異なる場合、最も制約の少ないインスティテューションの設定が使用されます。';
...
$string['institutionsettingsdescription'] = 'あなたが管理者のインスティテューションに関してこの人のメンバシップ設定を変更できます。';
...
$string['institutionusersinstructionsinvited2'] = '「 %1$s 」フィールドの一覧ではインスティテューションへの参加の招待が送信されてまだ受け入れまたは拒否していない人すべてが表示されます。あなたは特定の人を探すため検索ボックスを使用できます。インスティテューションへの招待を何人か取り消したい場合、最初に「 %1$s 」フィールドで選択して右矢印ボタンをクリックして「 %2$s 」フィールドに移動してください。「招待を取り消す」ボタンにより「 %2$s 」フィールド内の人のあなたのインスティテューションへの招待を取り消します。「 %1$s 」フィールドに表示されている他の人はまだあなたのインスティテューションに参加できます。';
...
$string['institutionuserserrortoomanyusers'] = 'これらの人は追加されませんでした。メンバ数はインスティテューションで許可される最大メンバ数を超えることはできません。あなたは少数の人を追加するか、インスティテューションから人を削除するか、サイト管理者に最大メンバ数の増加を依頼できます。';
$string['institutionuserserrortoomanyinvites'] = 'あなたの招待は送信されませんでした。既存のメンバ + 未処理の招待者数はインスティテューションで許可される最大メンバ数を超えることはできません。あなたは少数の人を招待するか、インスティテューションから人を削除するか、サイト管理者に最大メンバ数の増加を依頼できます。';
...
<p>登録を承認した場合、人にはアカウントの有効化に関して詳細なインストラクションを記載した通知が送信されます。人が有効化リンクをクリックした場合、インスティテューションのメンバとして追加されます。有効化リンクは24時間で有効期限となり人の詳細は削除されます。まだ登録したい場合、同じ手続きを再度開始する必要があります。</p>
...
$string['uneditableusers'] = 'あなたは編集することのできない %s アカウントを選択しました。アカウントはリストから取り除かれました。';
...
$string['someusersnotinauthinstanceinstitution'] = 'あなたが選択した人の一部はこの認証方法を使用するインスティテューションに属していません。';
...
$string['cli_pwreset_info'] = 'あなたはこのコマンドラインPHPスクリプトで人のパスワードをリセットできます。これはパスワードのリセットが許可されている認証方法のみ動作します (例:「内部」)。';
...
$string['cli_deleteinactiveusers_cleanusers'] = '同様に「usr」テーブルからアカウントの削除を試みます。';
