 ...
 * @updated    2015-10-27 15:30:48 UTC
...
$string['recaptchaonregisterformdesc'] = 'この設定を有効にした場合、ユーザは自分が人間であることを証明するため、新しいアカウントの自己登録時に<a href="http://recaptcha.org/">reCAPTCHA</a>テストを通過する必要があります。';
...
$string['requireregistrationconfirmdescription'] = 'この設定を有効にした場合、アクティブになる前に自己登録ユーザアカウントはインスティテューション管理者に承認される必要があります。そうでない場合、それぞれのインスティテューションでインスティテューションへの新しいアカウントの登録時に確認が必要かどうか決定することができます。';
...
$string['sitefilesaccessdescription'] = 'この設定を有効にした場合、ログインユーザはサブフォルダ内のサイトファイルにアクセスすることができるようになります。デフォルトではトップレベルのディレクトリのファイルのみアクセスすることができます。';
...
$string['institutionadminuserspagedescription1'] = 'ここであなたはどのユーザがインスティテューションを管理する者になるのか選択することができます。現在の管理者は「現在の管理者」フィールドに潜在的な管理者は「インスティテューションメンバー」フィールドに一覧表示されます。';
$string['adminusersupdated'] = '管理ユーザが更新されました。';
$string['staffuserspagedescription1'] = 'ここであなたはどのユーザがサイトのスタッフになるのか選択することができます。現在のスタッフは「インスティテューションスタッフ」フィールドに潜在的な管理者は「潜在的なスタッフ」フィールドに一覧表示されます。';
...
$string['institutionusersinstructionslastinstitution1'] = '「 %1$s 」フィールドのユーザ一覧ではあなたのインスティテューションへの参加をまだリクエストしていないユーザ、および選択されたインスティテューションを離れたユーザすべてが表示されます。表示されるユーザ数を減らすため、あなたは検索ボックスを使用することができます。ユーザをインスティテューションに参加するよう招待するには1名またはそれ以上のユーザを選択して右矢印をクリックすることにより、最初にユーザを「 %2$s 」フィールドに移動してください。「ユーザを招待する」ボタンにより、「 %2$s 」フィールド内のユーザすべてに招待状を送信します。招待を受け入れるまで、これらのユーザはインスティテューションと関連付けられません。';
$string['institutionusersinstructionsmembers1'] = '「 %1$s 」フィールドのユーザ一覧ではインスティテューションのメンバーすべてを表示します。表示されるユーザ数を減らすため、あなたは検索ボックスを使用することができます。インスティテューションからユーザを削除するには1名またはそれ以上のユーザを選択して右矢印をクリックすることにより、最初にユーザを「 %2$s 」フィールドに移動してください。あなたが選択したユーザは「 %2$s 」フィールドに移動されます。「ユーザを削除する」ボタンにより、「 %2$s 」フィールド内のユーザすべてをインスティテューションから削除します。';
$string['institutionusersinstructionsinvited1'] = '「 %1$s 」フィールドのユーザ一覧ではインスティテューションに参加するよう招待状が送信され、まだ承認または拒否していないユーザすべてが表示されます。表示されるユーザ数を減らすため、あなたは検索ボックスを使用することができます。インスティテューションへの招待を取り消すには1名またはそれ以上のユーザを選択して右矢印をクリックすることにより、最初にユーザを「 %2$s 」フィールドに移動してください。あなたが選択したユーザは「 %2$s 」フィールドに移動されます。「招待を取り消す」ボタンにより、「 %2$s 」フィールド内のユーザすべてに対する招待を削除します。他のユーザのインスティテューションへの招待は保持され、いつでも参加することができます。';
...
$string['masqueradingreasonrequireddescription1'] = 'この設定を有効にした場合、管理者は他のユーザとしてマスカレーディングする時に理由を入力する必要があります。この理由は記録されます。また、「ユーザにマスカレーディングを通知する」設定が有効にされた場合、マスカレーディングに関する理由を含んだ通知がユーザにも送信されます。マスカレーディングセッションのロギングを動作させるには「ロギング設定」を有効にする必要があります。';
$string['masqueradingnotified'] = 'ユーザにマスカレーディングを通知する';
$string['masqueradingnotifieddescription1'] = 'この設定を有効にした場合、管理者がユーザをマスカレーディングした時に通知されます。通知には誰が、いつ、なぜ (「マスカレーディングの理由を必要とする」が有効の場合) マスカレーディングしたのか記載されます。マスカレーディングセッションのロギングを動作させるには「ロギング設定」を有効にする必要があります。';
