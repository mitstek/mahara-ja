...
$string['newfiltersdescription'] = 'あなたが新しいHTMLフィルタをダウンロードした場合、%s フォルダ内にファイルを解凍した後、下のボタンをクリックしてインストールすることができます。';
...
$string['spamhausdescription'] = 'この設定を有効にした場合、Spamhaus DNSBLを使用して、URLがチェックされます。';
$string['staffuserreports'] = 'スタッフレポートアクセス';
$string['staffuserreportsdescription'] = 'この設定を有効にした場合、インスティテューション内ユーザのレポートページにサイトおよびインスティテューションスタッフがアクセスできるようになります。通常、このページは管理者に制限され、ページアクセスリストを含むユーザ詳細情報が一覧表示されます。';
$string['staffuserstats'] = 'スタッフ統計アクセス';
$string['staffuserstatsdescription'] = 'この設定を有効にした場合、インスティテューション内のユーザ統計ページにインスティテューションスタッフがアクセスできるようになります。通常、このページは管理者およびサイトスタッフのみに制限されています。';
$string['surbl'] = 'SURBL URLブラックリスト';
$string['surbldescription'] = 'この設定を有効にした場合、SURBL DNSBLを使用して、URLがチェックされます。';
...
$string['userscanchooseviewthemesdescription'] = 'この設定を有効にした場合、ユーザはページの編集時、テーマを選択することができます。選択したテーマを使用して、他のユーザにページが表示されます。';
$string['userscanhiderealnames'] = 'ユーザは実名を隠すことができる';
$string['userscanhiderealnamesdescription'] = 'この設定を有効にした場合、表示名を設定したユーザは表示名のみ検索対象として、実名を検索されないようにすることができます (サイト管理セクションでは常にユーザを実名で検索することができます)。';
...
$string['networkingpagedescription'] = 'Maharaのネットワーキング機能では同じサーバまたは別のサーバで動作しているMaharaまたはMoodleと連携することができます。ネットワーキングが有効にされた場合、MoodleまたはMaharaにログインしたユーザに対するシングルサインオンを設定することができます。';
...
$string['importfilemissinglisting'] = 'バルクエクスポートファイルのファイル名が「usernames.csv」ではありません。これらのユーザをエクスポートするため、あなたはMaharaバルクエクスポータを使用しましたか?';
...
$string['registrationalloweddescription3'] = 'この設定を有効にした場合、ユーザは登録フォームを使用して、このインスティテューションに登録することができます。登録が無効にされた場合、非メンバーはインスティテューションのメンバーシップをリクエストできません。同様にメンバーはインスティテューションから離脱できない、または自分のユーザアカウントを削除できないようになります。';
...
$string['institutionusersinstructionsnonmembers'] = '左側のユーザ一覧にはあなたのインスティテューションのメンバーではない、すべてのユーザが表示されます。あなたは表示されるユーザ数を減らすため、検索ボックスを使用することができます。あなたがユーザに対してインスティテューションへの参加を招待したい場合、1名またはそれ以上のユーザを選択して右矢印をクリックすることにより、最初にユーザを右側ボックスに移動してください。「ユーザを招待する」ボタンをクリックすることにより、右側ボックスのすべてのユーザに招待状を送信することができます。招待を受け入れるまで、これらのユーザはインスティテューションと関連付けされません。';
$string['institutionusersinstructionslastinstitution'] = '左側のユーザ一覧にはあなたのインスティテューションのメンバーではなく、選択されたインスティテューションを離れたすべてのユーザが表示されます。あなたは表示されるユーザ数を減らすため、検索ボックスを使用することができます。あなたがユーザに対してインスティテューションへの参加を招待したい場合、1名またはそれ以上のユーザを選択して右矢印をクリックすることにより、最初にユーザを右側ボックスに移動してください。「ユーザを招待する」ボタンをクリックすることにより、右側ボックスのすべてのユーザに招待状を送信することができます。招待を受け入れるまで、これらのユーザはインスティテューションと関連付けされません。';
$string['institutionusersinstructionsmembers'] = '左側のユーザ一覧にはあなたのインスティテューションのすべてのユーザが表示されます。あなたは表示されるユーザ数を減らすため、検索ボックスを使用することができます。ユーザをインスティテューションから削除するには最初に1名またはそれ以上のユーザを左側のボックスで選択した後、右矢印をクリックしてください。あなたが選択したユーザが右側のボックスに移動されます。「ユーザを削除する」ボタンをクリックすることにより、右側ボックスのすべてのユーザが削除されます。左側ボックスのユーザはインスティテューションに参加したままの状態です。';
$string['institutionusersinstructionsinvited'] = '左側のユーザ一覧にはインスティテューションへの参加の招待が送信されたすべてのユーザおよび誰が承認または拒否したか表示されます。あなたは表示されるユーザ数を減らすため、検索ボックスを使用することができます。インスティテューションへの招待を取り消すには最初に1名またはそれ以上のユーザを左側のボックスで選択した後、右矢印をクリックしてください。あなたが選択したユーザが右側のボックスに移動されます。「招待を取り消す」ボタンをクリックすることにより、右側ボックスのすべてのユーザへの招待が取り消されます。左側ボックスのユーザはインスティテューションに招待されたままの状態で、いつでもインスティテューションに参加することができます。';
...
$string['unsuspendinstitutiondescription'] = 'ここであなたはインスティテューションの利用停止を解除することができます。利用停止されたインスティテューションのユーザはインスティテューションが利用停止解除されるまでログインすることはできません。<br /><strong>注意:</strong> 有効期限をリセットまたは無効にしないまま利用停止解除することは結果として、毎日、再度利用停止されることにつながります。';
...
$string['masqueradingreasonrequireddescription'] = 'この設定を有効にした場合、管理者は他のユーザとしてマスカレーディングする時に理由を入力する必要があります。この理由は記録されます。また、「ユーザにマスカレーディングを通知する」設定が有効にされた場合、マスカレーディングに関する通知がユーザにも送信されます。';
$string['masqueradingnotified'] = 'ユーザにマスカレーディングを通知する';
$string['masqueradingnotifieddescription'] = 'この設定を有効にした場合、管理者がユーザをマスカレーディングした時に通知されます。通知には誰が、いつ、なぜ (「マスカレーディングの理由を必要とする」が有効の場合) マスカレーディングしたのか記載されます。';
