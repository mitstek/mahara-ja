...
$string['submitviewconfirm'] = 'あなたが「 %s 」を「 %s 」に評価のため送信する場合、チューターによるページの採点完了まであなたのページを編集できません。本当にこのページを送信してもよろしいですか?';
...
$string['retainviewrightsgroupdescription'] = 'このページが他のユーザによりコピーされた後、グループメンバーがこのページを閲覧できるようにしたい場合、このチェックボックスをチェックしてください。希望すればコピー後にユーザが閲覧アクセスを無効にできます。このページのコピーからコピーされたページは同じアクセスを保持することはありません。';
...
$string['choosetemplatepageandcollectiondescription'] = '<p>ここでは新しいページ作成の出発点としてあなたがコピーを許可されているページを検索できます。ページの名称をクリックすることであなたはそれぞれのページのプレビューを表示できます。あなたがコピーしたいページを見つけた場合、ページのコピーを作成するため該当する「ページをコピーする」ボタンをクリックしてコピーされたページの修正を開始してください。あなたは関連する「コレクションをコピーする」ボタンをクリックしてページが属しているコレクション全体コピーできます。</p>';
